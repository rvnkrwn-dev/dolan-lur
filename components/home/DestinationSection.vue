<template>
  <div class="mt-20 max-w-7xl mx-auto px-4">
    ...
    <template v-else>
        ...
        <NuxtLink v-for="item in wisata" :key="item.id" :to="`/destinations/${item?.slug}`"
          ...
          <div class="relative h-52">
            ...
            <div class="absolute bottom-2 left-2">
              ...
              <span class="text-xs text-white bg-gray-500/30 rounded-full py-1 px-2">{{ formatToIDR(item?.harga ?? 0) }}</span>
            </div>
          </div>
      ...
      <NuxtLink to="/destinations" class="group self-center h-fit w-fit mx-auto mt-4 relative block rounded-xl border border-gray-200 hover:border-transparent hover:shadow-lg focus:outline-none focus:border-transparent focus:shadow-lg transition-all">
